...
import dev.minimalist.ui.TierPicker
...
fun AppRulesScreen() {
    ...
    MinimalPage(title = t("Apps and limits"), subtitle = t("%s apps · anything you installed yourself starts gated", rules.size)) {
...
private fun AppRuleDetail(rule: AppRuleEntity, onDone: () -> Unit) {
    ...
    MinimalPage(title = rule.label, subtitle = rule.packageName) {
...
private fun StepperButton(label: String, modifier: Modifier = Modifier, onClick: () -> Unit) {
    Text(
        ...
        modifier = modifier
            .clip(MinimalShape)
            .border(1.dp, Fainter, MinimalShape)
