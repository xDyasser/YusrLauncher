...
fun BlackoutScreen() {
    ...
    MinimalPage(title = t("Blackout windows"), subtitle = t("adding one is instant; removing one waits out the cooldown")) {
        if (windows.isEmpty()) {
            Text(
        ...
        Column(modifier = Modifier.padding(top = 28.dp)) {
            ...
            MinimalButton(label = t("sleep") + " · 22:00–07:00 " + t("every day"), modifier = Modifier.padding(top = 12.dp)) {
                scope.launch { report(mutator.upsertBlackout(window(t("sleep"), 22 * 60, 7 * 60, EVERY_DAY))) }
            }
            MinimalButton(label = t("work") + " · 09:00–17:00 " + t("weekdays"), modifier = Modifier.padding(top = 12.dp)) {
                scope.launch { report(mutator.upsertBlackout(window(t("work"), 9 * 60, 17 * 60, WEEKDAYS))) }
            }
            MinimalButton(label = t("evening") + " · 19:00–22:00 " + t("every day"), modifier = Modifier.padding(top = 12.dp)) {
